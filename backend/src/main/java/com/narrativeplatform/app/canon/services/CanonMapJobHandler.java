...
public class CanonMapJobHandler implements AiJobTypeHandler {
    ...
    public String buildPrompt(final ChronicleEntity chronicle, final List<GameSegmentEntity> activeSegments) {
        ...
        final var enabledCategories = categories.stream()
                .filter(CanonMapGenerationCategoryEntity::isEnabled)
                .map(CanonMapGenerationCategoryEntity::getCategory)
                .map(Enum::name)
        ...
        builder.append("""
                ...
                - Only extract items belonging to these enabled categories: """)
                ...
                - Return only valid JSON: an object whose keys are exactly the enabled category names above, each mapping to an array of items (empty array if none). Each item: name, summary, visualDescription, personalityDescription (or null), visualBasis, personalityBasis (or null), sourceSegmentPositions.
    ...
    public void onCompleted(final AiJobEntity job, final AiRawGenerationResult raw) {
        ...
        final var enabledCategories = categorySnapshots.stream()
                .filter(CanonMapGenerationCategoryEntity::isEnabled)
                .map(CanonMapGenerationCategoryEntity::getCategory)
                .collect(Collectors.toSet());
        final Map<CanonCategoryType, CanonMapGenerationCategoryEntity> categoryByType = new EnumMap<>(CanonCategoryType.class);
        categorySnapshots.forEach(snapshot -> categoryByType.put(snapshot.getCategory(), snapshot));
        ...
        final var validated = canonMapValidationService.validate(raw.text(), enabledCategories, validPositions);
        for (final var entry : validated.entrySet()) {
            final var categorySnapshot = categoryByType.get(entry.getKey());
            var order = 0;
            for (final var validatedTag : entry.getValue()) {
                final var tag = canonTagRepository.save(new CanonTagEntity(
                        generation, entry.getKey(), categorySnapshot.getColor(), order++,
